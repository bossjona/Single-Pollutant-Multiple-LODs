...
gen.sim.plot <- function(design, large){
  ...
  if(design == "Cohort" & large == TRUE){
    ...
    depbatchcov_withind <- read.xlsx("largecohort_depbatchcov_withind.xlsx", sheetIndex = 1)
    depbatchcov_withoutind <- read.xlsx("largecohort_depbatchcov_withoutind.xlsx", sheetIndex = 1)
    ...
    depbatchcov_withind <- read.xlsx("modcohort_depbatchcov_withind.xlsx", sheetIndex = 1)
    depbatchcov_withoutind <- read.xlsx("modcohort_depbatchcov_withoutind.xlsx", sheetIndex = 1)
  ...
  bias_depbatch_withoutind_tmp <- depbatch_withoutind[, names(depbatch_withoutind) %in%
                                                        c("lod_name","gs_rel_bias_wob","cca_rel_bias_wob","sq2_rel_bias_wob","mice_rel_bias_wob","pmi_rel_bias_wob")]
  if(design == "Cohort"){
    bias_depbatchcov_withind_tmp <- depbatchcov_withind[, names(depbatchcov_withind) %in%
                                                    c("lod_name","gs_rel_bias","cca_rel_bias","sq2_rel_bias","mice_rel_bias","pmi_rel_bias")]
    bias_depbatchcov_withoutind_tmp <- depbatchcov_withoutind[, names(depbatchcov_withoutind) %in%
                                                          c("lod_name","gs_rel_bias_wob","cca_rel_bias_wob","sq2_rel_bias_wob","mice_rel_bias_wob","pmi_rel_bias_wob")]
  }
  #Stack Bias
  ...
  bias_depbatch_withoutind <- gather(bias_depbatch_withoutind_tmp[, !(names(bias_depbatch_withoutind_tmp) %in% c("gs_rel_bias_wob"))],
                                     method, bias, cca_rel_bias_wob:pmi_rel_bias_wob, factor_key = TRUE)
  if(design == "Cohort"){
    bias_depbatchcov_withind <- gather(bias_depbatchcov_withind_tmp[, !(names(bias_depbatchcov_withind_tmp) %in% c("gs_rel_bias"))],
                                    method, bias, cca_rel_bias:pmi_rel_bias, factor_key = TRUE)
    bias_depbatchcov_withoutind <- gather(bias_depbatchcov_withoutind_tmp[, !(names(bias_depbatchcov_withoutind_tmp) %in% c("gs_rel_bias_wob"))],
                                       method, bias, cca_rel_bias_wob:pmi_rel_bias_wob, factor_key = TRUE)
  }
  #Covert to numeric
  ...
  if(design == "Cohort"){
    bias_depbatchcov_withind$bias <- as.numeric(bias_depbatchcov_withind$bias)
    bias_depbatchcov_withoutind$bias <- as.numeric(bias_depbatchcov_withoutind$bias)
  }
  
  #Determine y-axis range and ticks
  if(design == "Cohort" & large == TRUE){
    bias_bounds <- c(-70, 60, 10)
    mse_bounds <- c(0, 0.07, 0.01)
  } else if(design == "Cohort" & large == FALSE){
    bias_bounds <- c(-70, 60, 10)
    mse_bounds <- c(0, 0.09, 0.01)
  } else if(design == "Case Control" & large == TRUE){
  ...
  b1 <- ggplot(data = bias_randbatch_withoutind, aes(x = lod_name, y = bias, color = factor(method), shape = factor(method))) +
    geom_point(size = 3) +
    ggtitle("A)            Without Batch Indicator - Random") + 
  ...
  b2 <- ggplot(data = bias_randbatch_withind, aes(x = lod_name, y = bias, color = factor(method), shape = factor(method))) +
    geom_point(size = 3) +
    ggtitle("B)               With Batch Indicator - Random") + 
    ylab("") + xlab("") +
    #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
    #scale_color_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
    scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
  ...
  #For cohort study design, b3 and b4 are overwritten by the code inside the if statement.
  if(design == "Cohort"){
    b3 <- ggplot(data = bias_depbatch_withoutind, aes(x = lod_name, y = bias, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("C)   Without Batch Indicator - Outcome Dependent") + 
      ylab("Relative Bias (%)") + xlab("") +
      #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
      #scale_color_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
      scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
    ...
    b4 <- ggplot(data = bias_depbatch_withind, aes(x = lod_name, y = bias, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("D)       With Batch Indicator - Outcome Dependent") + 
      ylab("") + xlab("") +
      #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
      #scale_color_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
      scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
    ...
    b5 <- ggplot(data = bias_depbatchcov_withoutind, aes(x = lod_name, y = bias, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("E)   Without Batch Indicator - Covariate Dependent") + 
      ylab("Relative Bias (%)") + xlab("LOD Pairs") +
      #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
      #scale_color_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
      scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
    ...
    b6 <- ggplot(data = bias_depbatchcov_withind, aes(x = lod_name, y = bias, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("F)      With Batch Indicator - Covariate Dependent") + 
      ylab("") + xlab("LOD Pairs") +
      #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
      #scale_color_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
      scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
  ...
  #Save paneled relative bias graphs
  ...
  if(design == "Cohort" & large == TRUE){
    png(file = "./grid_large_cohort_rel_bias.png", width = 9, height = 12, res = 400, units = 'in')
    grid_arrange_shared_legend(b1,b2,b3,b4,b5,b6, ncol = 2, nrow = 3)
    dev.off()
  } else if(design == "Cohort" & large == FALSE){
    pdf(file = "./grid_mod_cohort_rel_bias.pdf", width = 9, height = 12, onefile=FALSE)
    grid_arrange_shared_legend(b1,b2,b3,b4,b5,b6, ncol = 2, nrow = 3)
    dev.off()
  } else if(design == "Case Control" & large == TRUE){
    ...
    png(file = "./grid_mod_cc_rel_bias.png", width = 9, height = 9, res = 400, units = 'in')
    grid_arrange_shared_legend(b1,b2,b3,b4, ncol = 2, nrow = 2)
    dev.off()
  }
  ...
  mse_depbatch_withoutind_tmp <- depbatch_withoutind[, names(depbatch_withoutind) %in%
                                                        c("lod_name","gs_mse_wob","cca_mse_wob","sq2_mse_wob","mice_mse_wob","pmi_mse_wob")]
  if(design == "Cohort"){
    mse_depbatchcov_withind_tmp <- depbatchcov_withind[, names(depbatchcov_withind) %in%
                                                   c("lod_name","gs_mse","cca_mse","sq2_mse","mice_mse","pmi_mse")]
    mse_depbatchcov_withoutind_tmp <- depbatchcov_withoutind[, names(depbatchcov_withoutind) %in%
                                                         c("lod_name","gs_mse_wob","cca_mse_wob","sq2_mse_wob","mice_mse_wob","pmi_mse_wob")]
  }
  ...
  mse_depbatch_withoutind <- gather(mse_depbatch_withoutind_tmp[, !(names(mse_depbatch_withoutind_tmp) %in% c("gs_mse_wob"))],
                                     method, mse, cca_mse_wob:pmi_mse_wob, factor_key = TRUE)
  if(design == "Cohort"){
    mse_depbatchcov_withind <- gather(mse_depbatchcov_withind_tmp[, !(names(mse_depbatchcov_withind_tmp) %in% c("gs_mse"))],
                                   method, mse, cca_mse:pmi_mse, factor_key = TRUE)
    mse_depbatchcov_withoutind <- gather(mse_depbatchcov_withoutind_tmp[, !(names(mse_depbatchcov_withoutind_tmp) %in% c("gs_mse_wob"))],
                                      method, mse, cca_mse_wob:pmi_mse_wob, factor_key = TRUE)
  }
  ...
  if(design == "Cohort"){
    mse_depbatchcov_withind$mse <- as.numeric(mse_depbatchcov_withind$mse)
    mse_depbatchcov_withoutind$mse <- as.numeric(mse_depbatchcov_withoutind$mse)
  }
  ...
  if(design == "Cohort"){
    comp5 <- as.numeric(as.character(depbatchcov_withoutind$gs_mse))[1]
    comp6 <- as.numeric(as.character(depbatchcov_withind$gs_mse))[1]
  }
  ...
  #For cohort study design, m3 and m4 are overwritten by the code inside the if statement.
  if(design == "Cohort"){
    m3 <- ggplot(data = mse_depbatch_withoutind, aes(x = lod_name, y = mse, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("C)   Without Batch Indicator - Outcome Dependent") + 
    ...
    m5 <- ggplot(data = mse_depbatchcov_withoutind, aes(x = lod_name, y = mse, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("E)   Without Batch Indicator - Covariate Dependent") + 
      ylab("MSE") + xlab("LOD Pairs") +
      #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
      ...
      scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
      scale_color_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
      geom_hline(yintercept = comp5) +
      scale_y_continuous(breaks=round(seq(from = mse_bounds[1], to = mse_bounds[2], by = mse_bounds[3]), digits = 4), limits=c(mse_bounds[1],mse_bounds[2])) +
      theme_bw() +
      theme(axis.text.x  = element_text(angle=45, vjust=0.5), plot.title = element_text(hjust = -0.3, vjust = 2, size = 12))
    
    m6 <- ggplot(data = mse_depbatch_withind, aes(x = lod_name, y = mse, color = factor(method), shape = factor(method))) +
      geom_point(size = 3) +
      ggtitle("F)      With Batch Indicator - Covariate Dependent") + 
      ylab("") + xlab("LOD Pairs") +
      #scale_shape_manual(name = "Method:", labels = c("CCA", expression(LOD/sqrt(2)), "MICE", "CLMI"), values = c(15,16,17,18)) +
      ...
      scale_shape_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c(15,16,17,18)) +
      scale_color_manual(name = "Method:", labels = c("CCA", "SQ2", "MICE", "CLMI"), values = c("blue","forestgreen","red","goldenrod")) +
      geom_hline(yintercept = comp6) +
      scale_y_continuous(breaks=round(seq(from = mse_bounds[1], to = mse_bounds[2], by = mse_bounds[3]), digits = 4), limits=c(mse_bounds[1],mse_bounds[2])) +
      theme_bw() +
      theme(axis.text.x  = element_text(angle=45, vjust=0.5), plot.title = element_text(hjust = -0.3, vjust = 2, size = 12))
  }
  
  #Save paneled MSE graphs
  
  setwd("C:\\Users\\John Boss\\Desktop\\Apples and Oranges Backup\\Jonathan\\Documents\\GSRA Presentations\\Single Pollutant Multiple LOD\\Final Results - Updated 6-1-18\\Final Plots")
  
  if(design == "Cohort" & large == TRUE){
    png(file = "./grid_large_cohort_mse.png", width = 9, height = 12, res = 400, units = 'in')
    grid_arrange_shared_legend(m1,m2,m3,m4,m5,m6, ncol = 2, nrow = 3)
    dev.off()
  } else if(design == "Cohort" & large == FALSE){
    pdf(file = "./grid_mod_cohort_mse.pdf", width = 9, height = 12, onefile=FALSE)
    grid_arrange_shared_legend(m1,m2,m3,m4,m5,m6, ncol = 2, nrow = 3)
    dev.off()
  } else if(design == "Case Control" & large == TRUE){
    ...
    png(file = "./grid_mod_cc_mse.png", width = 9, height = 9, res = 400, units = 'in')
    grid_arrange_shared_legend(m1,m2,m3,m4, ncol = 2, nrow = 2)
    dev.off()
  }
